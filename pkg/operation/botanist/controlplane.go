...
func (b *Botanist) DeleteKubeAPIServer(ctx context.Context) error {
	...
	if err := b.K8sSeedClient.Client().Delete(ctx, deploy, kubernetes.DefaultDeleteOptions...); client.IgnoreNotFound(err) != nil {
		return err
	}
...
func (b *Botanist) DeployKubeAPIServerService(ctx context.Context) error {
	...
	return b.K8sSeedClient.ChartApplier().Apply(ctx, filepath.Join(chartPathControlPlane, name), b.Shoot.SeedNamespace, name, kubernetes.Values(values))
}
