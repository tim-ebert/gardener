...
func (c *defaultPlantControl) reconcile(ctx context.Context, plant *gardencorev1beta1.Plant, logger logrus.FieldLogger) error {
	...
	if err != nil {
		if apierrors.IsNotFound(err) {
			return updateStatusToUnknown(ctx, gardenClient.Client(), plant, "Referenced Plant secret could not be found.", conditionAPIServerAvailable, conditionEveryNodeReady)
		}
	...
	if err != nil {
		...
		logger.Error(msg)
		return updateStatusToUnknown(ctx, gardenClient.Client(), plant, msg, conditionAPIServerAvailable, conditionEveryNodeReady)
	}
	...
	return updateStatus(ctx, gardenClient.Client(), plant, cloudInfo, conditionAPIServerAvailable, conditionEveryNodeReady)
}
