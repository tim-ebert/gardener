...
func (f *PlantClientSetFactory) NewClientSet(ctx context.Context, k clientmap.ClientSetKey) (kubernetes.Interface, error) {
	...
	clientSet, err := NewClientFromSecret(ctx, gardenClient, key.Namespace, plant.Spec.SecretRef.Name,
	...
	if err != nil {
		return nil, fmt.Errorf("failed constructing new client for Plant %q: %w", key.Key(), err)
	}
