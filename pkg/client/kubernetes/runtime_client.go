...
// NewRuntimeClient creates a new client.client with the given config and options.
// It also creates a new client cache, which will be started immediately
func NewRuntimeClient(config *rest.Config, options client.Options, stopCh <-chan struct{}) (client.Client, error) {
	if err := setClientOptionsDefaults(config, &options); err != nil {
		return nil, err
	}

	return newRuntimeClient(config, options, stopCh)
}
...
func NewRuntimeCache(config *rest.Config, options cache.Options) (cache.Cache, error) {
	...
	return newRuntimeCache(config, options)
}
...
// NewRuntimeClientWithConfig returns a new controller runtime client from a config.
