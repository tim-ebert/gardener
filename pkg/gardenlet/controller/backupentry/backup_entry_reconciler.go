...
func (r *reconciler) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	if be.DeletionTimestamp != nil {
		return r.deleteBackupEntry(gardenClient.Client(), be)
	}
	// When a BackupEntry deletion timestamp is not set we need to create/reconcile the backup entry.
	return r.reconcileBackupEntry(gardenClient.Client(), be)
}

func (r *reconciler) reconcileBackupEntry(gardenClient client.Client, backupEntry *gardencorev1beta1.BackupEntry) (reconcile.Result, error) {
	backupEntryLogger := logger.NewFieldLogger(logger.Logger, "backupentry", backupEntry.Name)

	if err := controllerutils.EnsureFinalizer(r.ctx, gardenClient, backupEntry, gardencorev1beta1.GardenerName); err != nil {
		backupEntryLogger.Errorf("Failed to ensure gardener finalizer on backupentry: %+v", err)
		return reconcile.Result{}, err
	}

	if updateErr := r.updateBackupEntryStatusProcessing(gardenClient, backupEntry, "Reconciliation of Backup Entry state in progress.", 2); updateErr != nil {
		backupEntryLogger.Errorf("Could not update the status after reconciliation start: %+v", updateErr)
		return reconcile.Result{}, updateErr
	}

	seedClientSet, err := r.clientMap.GetClient(r.ctx, keys.ForSeedWithName(*backupEntry.Spec.SeedName))
	if err != nil {
		return reconcile.Result{}, fmt.Errorf("failed to get seed client: %w", err)
	}

	a := newActuator(gardenClient, seedClientSet.Client(), backupEntry, r.logger)
	if err := a.Reconcile(r.ctx); err != nil {
		...
		if updateErr := updateBackupEntryStatusError(r.ctx, gardenClient, backupEntry, reconcileErr.Description+" Operation will be retried.", reconcileErr); updateErr != nil {
	...
	if updateErr := updateBackupEntryStatusSucceeded(r.ctx, gardenClient, backupEntry, "Backup Entry has been successfully reconciled."); updateErr != nil {
		backupEntryLogger.Errorf("Could not update the Shoot status after reconciliation success: %+v", updateErr)
		return reconcile.Result{}, updateErr
	}

	if updateErr := controllerutils.RemoveGardenerOperationAnnotation(r.ctx, retry.DefaultBackoff, gardenClient, backupEntry); updateErr != nil {
...
func (r *reconciler) deleteBackupEntry(gardenClient client.Client, backupEntry *gardencorev1beta1.BackupEntry) (reconcile.Result, error) {
	...
	if present || time.Since(backupEntry.DeletionTimestamp.Local()) > gracePeriod {
		if updateErr := r.updateBackupEntryStatusProcessing(gardenClient, backupEntry, "Deletion of Backup Entry in progress.", 2); updateErr != nil {
		...
		if err := a.Delete(r.ctx); err != nil {
			...
			if updateErr := updateBackupEntryStatusError(r.ctx, gardenClient, backupEntry, deleteErr.Description+" Operation will be retried.", deleteErr); updateErr != nil {
				backupEntryLogger.Errorf("Could not update the BackupEntry status after deletion error: %+v", updateErr)
				return reconcile.Result{}, updateErr
			}
			return reconcile.Result{}, errors.New(deleteErr.Description)
		}
		if updateErr := updateBackupEntryStatusSucceeded(r.ctx, gardenClient, backupEntry, "Backup Entry has been successfully deleted."); updateErr != nil {
		...
		return reconcile.Result{}, controllerutils.RemoveGardenerFinalizer(r.ctx, gardenClient, backupEntry)
	}
	if updateErr := updateBackupEntryStatusPending(r.ctx, gardenClient, backupEntry, fmt.Sprintf("Deletion of backup entry is scheduled for %s", backupEntry.DeletionTimestamp.Time.Add(gracePeriod))); updateErr != nil {
		backupEntryLogger.Errorf("Could not update the BackupEntry status after deletion successful: %+v", updateErr)
		return reconcile.Result{}, updateErr
	}
	return reconcile.Result{}, nil
...
func (r *reconciler) updateBackupEntryStatusProcessing(c client.Client, be *gardencorev1beta1.BackupEntry, message string, progress int32) error {
	return kutil.TryUpdateStatus(r.ctx, retry.DefaultRetry, c, be, func() error {
...
func updateBackupEntryStatusError(ctx context.Context, c client.Client, be *gardencorev1beta1.BackupEntry, message string, lastError *gardencorev1beta1.LastError) error {
	return kutil.TryUpdateStatus(ctx, retry.DefaultRetry, c, be, func() error {
